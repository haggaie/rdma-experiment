...
מתוכנית המשתמש ראשית מועתק לזיכרון של ליבת מערכת ההפעלה (kernel), ורק לאחר מכן
...
וכך משפרת ביצועים לתוכניות רשת. הספרייה מממשת פרוטוקולי TCP/IP באמצעות גישה
ישירה לכרטיס הרשת, ומחליפה באופן שקוף את ממשקי המשתמש של מערכת ההפעלה.

### פסיקות (interrupts) לעומת דגימה (polling)

כאשר תהליך ממתין לביצוע פעולת קלט/פלט, כמו קבלת מידע מהרשת או שליחת מידע,
מערכת ההפעלה משהה את ריצת התהליך, ומאפשרת לתהליכים אחרים לנצל את המעבד.
כרטיס הרשת מודיע למערכת ההפעלה על אירועי קלט/פלט באמצעות פסיקה, אשר גורמת
למעבד להפסיק את ריצת התהליך הנוכחי, לעבד את החבילות שהתקבלו, ולסמן את התהליך
שממתין לתקשורת כ"מוכן לריצה". לאחר מכן, לפי החלטת סדרן התוכניות (Scheduler),
מתבצעת החלפת הקשר והתהליך המקורי ממשיך את הריצה שלו.

מנגנון פסיקות זה מאפשר ניצול יעיל של המעבד, על ידי השהיית תהליכים הממתינים
לקלט/פלט, וניצול המעבד בזמן ההמתנה לחישובים אחרים. עם זאת, למנגנון זה יש תקורה:
הזמן שלוקח מרגע קבלת חבילה ועד הגעת המידע לתהליך המשתמש גדול
יחסית. דרך אחרת לקבל מידע שמשתלבת עם מעקף מערכת ההפעלה היא להשתמש בדגימה (polling)
חוזרת ונשנית של מצב פעולת קלט/פלט. התהליך מבקש את ביצוע הפעולה
ואז אינו מוותר על זכות השימוש במעבד אלא ממשיך לדגום שוב ושוב האם הפעולה
הסתיימה. באופן זה מתקצרת ההשהייה של קבלת הודעות, אולם המעבד מנוצל ב-100%
לטובת תהליך הקלט/פלט וביצוע הדגימות. 

ספריית VMA בה נשתמש בניסוי מפעילה מנגנון דגימה שכזה כברירת מחדל.
...
### ריבוי ליבות (Multi-core Processing)
...
מערכת ההפעלה לינוקס מנסה לאזן את העומס על ליבות המעבד השונות (CPU cores), ולעתים זה
...
תרגיל 2: TCP ומעקף ליבת מערכת ההפעלה (Kernel Bypass)
------------------------------------
...
System Monitor.
בכלי `top` תוכלו להציג את ניצול כל ליבת מעבד (core) בנפרד באמצעות לחיצה על המקש <kbd>1</kbd>.
...
הלקוח), ובחרו גודל הודעה קטן (16 בתים) כדי למזער את השפעת זמן השידור על השהיית הרשת.
...
שימוש בספריית VMA, עבור אותם גדלי הודעות מן הניסוי הקודם (16 בתים להשהיית הרשת ו-1472 בתים לקצב ההעברה). מדדו גם את ניצולת המעבד. השוו את התוצאות והסבירו את
התוצאות שקיבלתם.
...
&lrm;`-s`)
של 16 בתים (העבירו את הפרמטר גם לשרת וגם ללקוח). מהו קצב ההעברה (מספר הודעות לשניה)
שהתקבל, ומדוע?
...
כאשר מחליפים את
&lrm;`$governor`
ב-`powersave` או `performance` לפי הצורך. כלי perftest עשויים להדפיס אזהרה כאשר תדר השעון של המעבד אינו תואם לתדר המקסימלי שלו:
...
מדדו את קצב השליחה של הודעות קצרות (16 בתים) כאשר לשרת תדר שעון נמוך ותדר גבוה והשוו את התוצאות כאשר
משתמשים בפעולות RDMA Write לעומת פעולות RDMA Send/Receive.

מדוע נרצה להשתמש בתדר שעון נמוך מהרגיל?

תרגיל 4: ריבוי ליבות (Multi-core Processing)
--------------------
...
[lstopo](#lstopo)
ובמבנה המערכת שמצאתם באמצעותה. יש להבחין בין ליבות נפרדות ובין חוטים החולקים 
את אותה ליבת מעבד באמצעות 
...
כדי לראות את השפעת מספר התהליכים על קצב ההעברה, שרטטו גרף המשווה בציר Y את קצב ההעברה 
(הודעות לשניה) אל מול מספר התהליכים (ציר X). הפעילו תהליך נפרד של `ib_send_bw` על כל אחת מהליבות בשרת (באמצעות
...
של לקוחות `ib_send_bw` כאשר כל לקוח פונה לשרת שונה. השתמשו בגודל הודעות קטן
(פרמטר
&lrm;`-s`) של 4 בתים. מדדו את סך קצב
ההעברה של כל הלקוחות (הודעות לשניה), ואת ניצולת המעבדים הכוללת.

כעת חזרו על הניסוי אבל תוך שימוש בגודל הודעה גדול (65536 בתים).
הסבירו את ההבדלים בין גדלי ההודעות השונים. מהו צוואר הבקבוק בכל ניסוי?
