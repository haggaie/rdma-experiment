...
במשתנה db\_info בקובץ
`rdma_client.c`.

ערכו את הפונקציה `generate_request` ב-`rdma_client.c`, כך שבמקום לשלוח הודעה אל
השרת, תשלח בקשה לקריאת זכרון מרוחק של השרת.

הערה: על מנת לנפות שגיאות, ניתן להשתמש בתוכנית gdb. כדי לבנות גרסה מתאימה של התוכנית ניתן לערוך את `build.sh` ולהוסיף את הפרמטר
...
אינטראקציה עם המשתמש (יש לבטל הדפסות בתוכנית), ומספר רב של פעולות כתיבה. מדדו את הזמן שלוקח לבצע
את סוגי הפעולות השונים (למשל באמצעות הפקודה
&lrm;`time <cmd>`&lrm;),
והשוו ביניהם.
