...
מעורבות של מערכת ההפעלה. באמצעות מערכת מיפוי זיכרון מקבילה ל-MMU של
המעבד הראשי, הכרטיס משתמש בזיכרון וירטואלי ומסוגל להפריד בין תהליכים
שונים. כך תוכנית המשתמש יכולה לתת הוראות קבלה או שליחה עם כתובות במרחב
...
זה אנחנו מתמקדים בפרוטוקולים ממשפחת RoCE – RDMA over Converged Ethernet.

כאשר שולחים או מקבלים הודעות, פרוטוקול Reliable Connected (RC) ממשפחת
RoCE מאפשר לתוכנית המשתמש לספק הודעות גדולות לשליחה (עד 2GB), או חוצצים
...
לזיכרון מרוחק. אנחנו מתייחסים לתחנות הקצה בתור היוזם והיעד. אם בתקשורת
דו-צדדית הצד היוזם שולח הודעה אל היעד, והיעד מספק חוצץ זכרון לקבלת
ההודעה ומקבל התראה על קבלתה, כאשר משתמשים ב-RDMA התקשורת הינה חד-צדדית.
היוזם שולח הוראה לבצע כתיבה או קריאה של זיכרון השייך ליעד, וכרטיס הרשת
...
-   פרק 5 RDMA Verbs API. הפונקציות הרלוונטיות הן:
    ...
    -   rdma\_get\_send\_completion

    -   rdma\_get\_recv\_completion
...
המחובר יסומן עם הדגל UP, ואילו המנותק יופיע עם הדגל NO-CARRIER.
...
לשם כך נשתמש בכלי taskset. הכלי מקבל פרמטר -c ולאחריו את מספר הליבה
...
לשלוט במשך זמן הניסוי באמצעות הפרמטר -t.
...
רק לאחר קבלת ההודעה הקודמת. כך מדידת זמן ההעברה הכולל מאפשר למדוד את
השהיית הרשת בין שתי התחנות. השתמשו בפרוטוקול UDP המתאים יותר למדידת
השהייה על ידי השמטת הפרמטר –tcp משורת הפקודה של sockperf (בצד השרת ובצד
...
משתנה הסביבה LD\_PRELOAD=libvma.so. כדי לקבל הרשאה לגישה ישירה לכרטיס
...
-s) של 4 בתים (העבירו את הפרמטר גם לשרת וגם ללקוח). מהו קצב ההעברה
שהתקבל, ומדוע?

כדי למדוד את השהיית הרשת, נשתמש בכלי ib\_send\_lat, הפועל באופן דומה.
...
הזכרון. נדגים זאת באמצעות תוכנית הבדיקה ib\_write\_bw, בה תוכנית הלקוח
מבצעת כתיבות לזכרון של תוכנית השרת. אופן הפעולה שלה זהה לשאר תוכניות
perftest.

מדדו את קצב העברת הנתונים באמצעות ib\_write\_bw. מדדו במהלך הריצה את
...
ביצועים וקצב העברה גבוה יותר באמצעות ניצול מספר ליבות של המעבד. בדקו
באמצעות הפקודה numactl --hardware כמה מעבדים במערכת שברשותכם, וכיצד הם
מאורגנים.
...
בפרמטר -p על מנת לבחור פורט שונה לכל תהליך. באופן דומה, הפעילו עותק נפרד
של לקוחות ib\_send\_bw כאשר כל לקוח פונה לשרת שונה. מדדו את סך קצב
ההעברה של כל הלקוחות, ואת ניצולת המעבדים הכוללת.

כעת חזרו על הניסוי אבל תוך שימוש בגודל הודעה קטן (השתמשו בפרמטר -s) של 4
בתים. בדקו את הקצבים המתקבלים עם מספר ליבות בין 1 ל-4, וערכו תרשים
...
בהינתן המפתח המתאים. הדוגמה מורכבת מתוכנית שרת rdma\_server, המקבלת
בקשות כתיבה וקריאה של נתונים ושומרת אותם במערך בזיכרון, ותוכנית לקוח
rdma\_client המאפשרת למשתמש לשמור ולקרוא מפתחות בצורה אינטראקטיבית.

על מנת לבנות את התוכנית, הפעילו את הסקריפט build.sh הנמצא בתיקיה
rdma-experiment/src. לאחר מכן התוכניות המוכנות צריכות להמצא בתיקיה
...
את האובייקטים הבאים:
  ...
  ---------------------------------- ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  מזהה חיבור rdma\_cm\_id (ID)       מזהה את החיבור עם הצד השני ברשת, ומאפשר לבצע מולו פעולות של שליחה וקבלה.
  ...
  זכרון רשום memory region (MR)      כדי לאפשר לכרטיס הרשת גישה ישירה לזכרון, יש לרשום את הזכרון המבוקש מראש. לאחר הרישום מתקבל אוביקט MR המייצג את הזכרון הרשום.

  השלמת משימה work completion (WC)   לאחר ביצוע פעולת שליחה או קבלה בהצלחה, מתווספים נתוני השלמת משימה לתור פנימי המשוייך ל-ID. התוכנית יכולה לקרוא את הנתונים מהתור וכך לזהות שהפעולה הושלמה. רק לאחר השלמת הפעולה וקבלת הנתונים מותר לתוכנית לעשות שימוש בזכרון המשוייך לפעולה.
  ---------------------------------- ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
על מנת לשלוח פעולת קריאה, יש להשתמש בפונקציה rdma\_post\_read. פעולה זו
...
במשתנה db\_info בקובץ client.c.

ערכו את הפונקציה generate\_request ב-client.c, כך שבמקום לשלוח הודעה אל
השרת, תשלח בקשה לקריאת זכרון מרוחק של השרת.

### מדידת ביצועים

שנו את תוכנית הלקוח כך שתבצע מספר רב של פעולות קריאה (מליון), ללא
אינטראקציה עם המשתמש, ומספר רב של פעולות כתיבה. מדדו את הזמן שלוקח לבצע
את סוגי הפעולות השונים, והשוו ביניהם.
